...
async def get_overview_stats(
    ...
    daily_stats = db.query(
        ...
        func.date_trunc('day', models.ContentQueueItem.created_at)
    ).all()
    
    return {
        "content_stats": content_stats,
        "platform_distribution": {str(p.source_platform): p.count for p in platform_stats},
        "daily_activity": [{"date": str(d.date), "count": d.count} for d in daily_stats]
    }
...
async def get_performance_stats(
    ...
    error_rates = db.query(
        ...
        func.sum(
            case(
                [(models.ContentQueueItem.status == models.ContentStatus.FAILED, 1)],
                else_=0
            )
        ).label('failed')
    ).filter(
...
async def get_destination_stats(
    ...
    account_stats = db.query(
        ...
        models.ContentQueueItem.destination_account_id == models.DestinationAccount.id
    ).filter(
        models.ContentQueueItem.created_at >= start_date,
        models.ContentQueueItem.status == models.ContentStatus.COMPLETED
    ).group_by(
